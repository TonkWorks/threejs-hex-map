...
export function SelectionParticles(
  scene: Scene,
  position: Vector3,
  radius: number = 0.65
): ParticleSystem {
  ...
  const selectParticle = new ParticleSystem(scene, {
    ...
    onSpawn: (system, deltaTime) => {
      ...
      for (let i = 0; i < spawnCount; i++) {
        ...
        const offset = new Vector2().lerpVectors(currentVertex, nextVertex, t);
        ...
        system.addParticle({
          position: new Vector3(
            system.options.spawnPosition.x + offset.x,
            system.options.spawnPosition.y + offset.y,
            system.options.spawnPosition.z
          ),
          velocity: new Vector3(0, 0, 0), // velocity not used in update
...
export class MeshSurfaceSampler {
    ...
    constructor(mesh: Mesh) {
      this.mesh = mesh;
      this.geometry = mesh.geometry as BufferGeometry;
      this.build();
    }
...
export class MainParticleEffects {
    ...
    public static createHexSelection(position: Vector3, scene: Scene): ParticleSystem {
        // Hex selection effect - glowing border with light pulse
        const hexSelectionSystem = new ParticleSystem(scene, {
            maxParticles: 600,
            particleSize: 80,
            ...
            onSpawn: (system, deltaTime) => {
                ...
                for (let i = 0; i < spawnCount; i++) {
                    ...
                    system.addParticle({
                        ...
                        startColor: new Color(0.9, 0.8, 0.2),  // Golden glow
                        endColor: new Color(0.9, 0.5, 0.1),    // Fades to orange
        ...
        ParticleSystems.push(hexSelectionSystem);
                ...
                for (let i = 0; i < spawnCount; i++) {
                    ...
                    if (particleType === 'beam') {
                        ...
                        system.addParticle({
                            ...
                            startColor: new Color(0, 0, 1),  // Bright blue
                            endColor: new Color(0.7, 0.9, 1.0),    // Fade to white-blue
    ...
    public static createBuildEffect(position: Vector3, scene: Scene): void {
