...
define(["require", "exports", "./constants.js", "../../../../build/three.module.js", "./WebGPUTextureUtils.js"], function (require, exports, constants_js_1, three_module_js_1, WebGPUTextureUtils_js_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    WebGPUTextureUtils_js_1 = __importDefault(WebGPUTextureUtils_js_1);
    class WebGPUTextures {
        ...
        getDefaultTexture() {
            if (this.defaultTexture === null) {
                const texture = new three_module_js_1.Texture();
        ...
        getDefaultCubeTexture() {
            if (this.defaultCubeTexture === null) {
                const texture = new three_module_js_1.CubeTexture();
                texture.minFilter = three_module_js_1.NearestFilter;
                texture.magFilter = three_module_js_1.NearestFilter;
        ...
        initRenderTarget(renderTarget) {
            ...
            if (renderTargetProperties.initialized === undefined) {
                ...
                if (renderTarget.depthBuffer === true) {
                    const depthTextureFormat = constants_js_1.GPUTextureFormat.Depth24PlusStencil8; // @TODO: Make configurable
        ...
        _convertAddressMode(value) {
            let addressMode = constants_js_1.GPUAddressMode.ClampToEdge;
            if (value === three_module_js_1.RepeatWrapping) {
                addressMode = constants_js_1.GPUAddressMode.Repeat;
            }
            else if (value === three_module_js_1.MirroredRepeatWrapping) {
                addressMode = constants_js_1.GPUAddressMode.MirrorRepeat;
            }
            return addressMode;
        }
        _convertFilterMode(value) {
            let filterMode = constants_js_1.GPUFilterMode.Linear;
            if (value === three_module_js_1.NearestFilter || value === three_module_js_1.NearestMipmapNearestFilter || value === three_module_js_1.NearestMipmapLinearFilter) {
                filterMode = constants_js_1.GPUFilterMode.Nearest;
            }
            return filterMode;
        }
        ...
        _generateMipmaps(textureGPU, textureGPUDescriptor) {
            if (this.utils === null) {
                this.utils = new WebGPUTextureUtils_js_1.default(this.device, this.glslang); // only create this helper if necessary
            }
            this.utils.generateMipmaps(textureGPU, textureGPUDescriptor);
        }
        _getBlockData(format) {
            // this method is only relevant for compressed texture formats
            if (format === constants_js_1.GPUTextureFormat.BC1RGBAUnorm || format === constants_js_1.GPUTextureFormat.BC1RGBAUnormSRGB)
                return { byteLength: 8, width: 4, height: 4 }; // DXT1
            if (format === constants_js_1.GPUTextureFormat.BC2RGBAUnorm || format === constants_js_1.GPUTextureFormat.BC2RGBAUnormSRGB)
                return { byteLength: 16, width: 4, height: 4 }; // DXT3
            if (format === constants_js_1.GPUTextureFormat.BC3RGBAUnorm || format === constants_js_1.GPUTextureFormat.BC3RGBAUnormSRGB)
                return { byteLength: 16, width: 4, height: 4 }; // DXT5
            if (format === constants_js_1.GPUTextureFormat.BC4RUnorm || format === constants_js_1.GPUTextureFormat.BC4RSNorm)
                return { byteLength: 8, width: 4, height: 4 }; // RGTC1
            if (format === constants_js_1.GPUTextureFormat.BC5RGUnorm || format === constants_js_1.GPUTextureFormat.BC5RGSnorm)
                return { byteLength: 16, width: 4, height: 4 }; // RGTC2
            if (format === constants_js_1.GPUTextureFormat.BC6HRGBUFloat || format === constants_js_1.GPUTextureFormat.BC6HRGBFloat)
                return { byteLength: 16, width: 4, height: 4 }; // BPTC (float)
            if (format === constants_js_1.GPUTextureFormat.BC7RGBAUnorm || format === constants_js_1.GPUTextureFormat.BC7RGBAUnormSRGB)
                return { byteLength: 16, width: 4, height: 4 }; // BPTC (unorm)
        }
        _getBytesPerTexel(format) {
            if (format === constants_js_1.GPUTextureFormat.R8Unorm)
                return 1;
            if (format === constants_js_1.GPUTextureFormat.R16Float)
                return 2;
            if (format === constants_js_1.GPUTextureFormat.RG8Unorm)
                return 2;
            if (format === constants_js_1.GPUTextureFormat.RG16Float)
                return 4;
            if (format === constants_js_1.GPUTextureFormat.R32Float)
                return 4;
            if (format === constants_js_1.GPUTextureFormat.RGBA8Unorm || format === constants_js_1.GPUTextureFormat.RGBA8UnormSRGB)
                return 4;
            if (format === constants_js_1.GPUTextureFormat.RG32Float)
                return 8;
            if (format === constants_js_1.GPUTextureFormat.RGBA16Float)
                return 8;
            if (format === constants_js_1.GPUTextureFormat.RGBA32Float)
                return 16;
        }
        _getDimension(texture) {
            let dimension;
            if (texture.isDataTexture3D) {
                dimension = constants_js_1.GPUTextureDimension.ThreeD;
            }
            else {
                dimension = constants_js_1.GPUTextureDimension.TwoD;
            }
            return dimension;
        }
        _getFormat(texture) {
            ...
            let formatGPU;
            switch (format) {
                case three_module_js_1.RGBA_S3TC_DXT1_Format:
                    formatGPU = (encoding === three_module_js_1.sRGBEncoding) ? constants_js_1.GPUTextureFormat.BC1RGBAUnormSRGB : constants_js_1.GPUTextureFormat.BC1RGBAUnorm;
                    break;
                case three_module_js_1.RGBA_S3TC_DXT3_Format:
                    formatGPU = (encoding === three_module_js_1.sRGBEncoding) ? constants_js_1.GPUTextureFormat.BC2RGBAUnormSRGB : constants_js_1.GPUTextureFormat.BC2RGBAUnorm;
                    break;
                case three_module_js_1.RGBA_S3TC_DXT5_Format:
                    formatGPU = (encoding === three_module_js_1.sRGBEncoding) ? constants_js_1.GPUTextureFormat.BC3RGBAUnormSRGB : constants_js_1.GPUTextureFormat.BC3RGBAUnorm;
                    break;
                case three_module_js_1.RGBFormat:
                case three_module_js_1.RGBAFormat:
                    switch (type) {
                        case three_module_js_1.UnsignedByteType:
                            formatGPU = (encoding === three_module_js_1.sRGBEncoding) ? constants_js_1.GPUTextureFormat.RGBA8UnormSRGB : constants_js_1.GPUTextureFormat.RGBA8Unorm;
                            break;
                        case three_module_js_1.HalfFloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.RGBA16Float;
                            break;
                        case three_module_js_1.FloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.RGBA32Float;
                            break;
                        default:
                            console.error('WebGPURenderer: Unsupported texture type with RGBAFormat.', type);
                    }
                    break;
                case three_module_js_1.RedFormat:
                    switch (type) {
                        case three_module_js_1.UnsignedByteType:
                            formatGPU = constants_js_1.GPUTextureFormat.R8Unorm;
                            break;
                        case three_module_js_1.HalfFloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.R16Float;
                            break;
                        case three_module_js_1.FloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.R32Float;
                            break;
                        default:
                            console.error('WebGPURenderer: Unsupported texture type with RedFormat.', type);
                    }
                    break;
                case three_module_js_1.RGFormat:
                    switch (type) {
                        case three_module_js_1.UnsignedByteType:
                            formatGPU = constants_js_1.GPUTextureFormat.RG8Unorm;
                            break;
                        case three_module_js_1.HalfFloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.RG16Float;
                            break;
                        case three_module_js_1.FloatType:
                            formatGPU = constants_js_1.GPUTextureFormat.RG32Float;
                            break;
                        default:
        ...
        _needsMipmaps(texture) {
            return (texture.isCompressedTexture !== true) && (texture.generateMipmaps === true) && (texture.minFilter !== three_module_js_1.NearestFilter) && (texture.minFilter !== three_module_js_1.LinearFilter);
        }
    }
    ...
    exports.default = WebGPUTextures;
});
//# sourceMappingURL=WebGPUTextures.js.map