define(["require", "exports", "./TempNode.js", "./NodeLib.js"], function (require, exports, TempNode_js_1, NodeLib_js_1) {
    "use strict";
    ...
    function FunctionNode(src, includes, extensions, keywords, type) {
        ...
        TempNode_js_1.TempNode.call(this, type);
        this.parse(src, includes, extensions, keywords);
    }
    exports.FunctionNode = FunctionNode;
    FunctionNode.prototype = Object.create(TempNode_js_1.TempNode.prototype);
    ...
    FunctionNode.prototype.generate = function (builder, output) {
        ...
        for (var i = 0; i < matches.length; i++) {
            ...
            if (this.keywords[prop] || (this.useKeywords && isGlobal && NodeLib_js_1.NodeLib.containsKeyword(prop))) {
                var node = this.keywords[prop];
                if (!node) {
                    var keyword = NodeLib_js_1.NodeLib.getKeywordData(prop);
                    if (keyword.cache)
                        node = builder.keywords[prop];
                    node = node || NodeLib_js_1.NodeLib.getKeyword(prop, builder);
                    if (keyword.cache)
            ...
            if (this.getIncludeByName(reference) === undefined && NodeLib_js_1.NodeLib.contains(reference)) {
                builder.include(NodeLib_js_1.NodeLib.get(reference));
            }
        }
        if (output === 'source') {
            return src;
