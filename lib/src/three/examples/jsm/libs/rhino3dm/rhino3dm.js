var rhino3dm = (function () {
    ...
    return (function (rhino3dm) {
        ...
        function addFunctionWasm(func, sig) { var table = wasmTable; var ret = table.length; try {
            table.grow(1);
        }
        catch (err) {
        ...
        var wasmMemory;
        var wasmTable = new WebAssembly.Table({ "initial": 6160, "maximum": 6160 + 0, "element": "anyfunc" });
        var ABORT = false;
        var EXITSTATUS = 0;
        ...
        var STACK_BASE = 5600800, DYNAMIC_BASE = 5600800, DYNAMICTOP_PTR = 357760;
        ...
        var FS = { root: null, mounts: [], devices: {}, streams: [], nextInode: 1, nameTable: null, currentPath: "/", initialized: false, ignorePermissions: true, trackingDelegate: {}, tracking: { openFlags: { READ: 1, WRITE: 2 } }, ErrnoError: null, genericErrors: {}, filesystems: null, syncFSRequests: 0, handleFSError: function (e) { if (!(e instanceof FS.ErrnoError))
            ...
            } var mounts = FS.getMounts(FS.root.mount); var completed = 0; function doCallback(err) { FS.syncFSRequests--; return callback(err); } function done(err) { if (err) {
                if (!done.errored) {
                    done.errored = true;
        ...
        var SYSCALLS = { DEFAULT_POLLMASK: 5, mappings: {}, umask: 511, calculateAt: function (dirfd, path) { if (path[0] !== "/") {
                ...
                if (curr < 0)
                    return -1;
                ret += curr;
            } return ret; }, varargs: 0, get: function (varargs) { SYSCALLS.varargs += 4; var ret = HEAP32[SYSCALLS.varargs - 4 >> 2]; return ret; }, getStr: function () { var ret = UTF8ToString(SYSCALLS.get()); return ret; }, getStreamFromFD: function (fd) { if (fd === undefined)
                fd = SYSCALLS.get(); var stream = FS.getStream(fd); if (!stream)
                throw new FS.ErrnoError(8); return stream; }, get64: function () { var low = SYSCALLS.get(), high = SYSCALLS.get(); return low; }, getZero: function () { SYSCALLS.get(); } };
        ...
        function __embind_finalize_value_object(structType) { var reg = structRegistrations[structType]; delete structRegistrations[structType]; var rawConstructor = reg.rawConstructor; var rawDestructor = reg.rawDestructor; var fieldRecords = reg.fields; var fieldTypes = fieldRecords.map(function (field) { return field.getterReturnType; }).concat(fieldRecords.map(function (field) { return field.setterArgumentType; })); whenDependentTypesAreResolved([structType], fieldTypes, function (fieldTypes) { var fields = {}; fieldRecords.forEach(function (field, i) { var fieldName = field.fieldName; var getterReturnType = fieldTypes[i]; var getter = field.getter; var getterContext = field.getterContext; var setterArgumentType = fieldTypes[i + fieldRecords.length]; var setter = field.setter; var setterContext = field.setterContext; fields[fieldName] = { read: function (ptr) { return getterReturnType["fromWireType"](getter(getterContext, ptr)); }, write: function (ptr, o) { var destructors = []; setter(setterContext, ptr, setterArgumentType["toWireType"](destructors, o)); runDestructors(destructors); } }; }); return [{ name: reg.name, "fromWireType": function (ptr) { var rv = {}; for (var i in fields) {
                    rv[i] = fields[i].read(ptr);
                } rawDestructor(ptr); return rv; }, "toWireType": function (destructors, o) { for (var fieldName in fields) {
                    if (!(fieldName in o)) {
        ...
        function __embind_register_std_wstring(rawType, charSize, name) { name = readLatin1String(name); var getHeap, shift; if (charSize === 2) {
            ...
            } _free(value); return a.join(""); }, "toWireType": function (destructors, value) { var length = value.length; var ptr = _malloc(4 + length * charSize); var HEAP = getHeap(); HEAPU32[ptr >> 2] = length; var start = ptr + 4 >> shift; for (var i = 0; i < length; ++i) {
        ...
        function __emval_allocateDestructors(destructorsRef) { var destructors = []; HEAP32[destructorsRef >> 2] = __emval_register(destructors); return destructors; }
        var emval_symbols = {};
        ...
        function __emval_call_method(caller, handle, methodName, destructorsRef, args) { caller = emval_methodCallers[caller]; handle = requireHandle(handle); methodName = getStringOrSymbol(methodName); return caller(handle, methodName, __emval_allocateDestructors(destructorsRef), args); }
        ...
        function __emval_equals(first, second) { first = requireHandle(first); second = requireHandle(second); return first == second; }
        ...
        function __emval_instanceof(object, constructor) { object = requireHandle(object); constructor = requireHandle(constructor); return object instanceof constructor; }
        function __emval_is_number(handle) { handle = requireHandle(handle); return typeof handle === "number"; }
        function __emval_is_string(handle) { handle = requireHandle(handle); return typeof handle === "string"; }
        ...
        function _emscripten_get_sbrk_ptr() { return 357760; }
        ...
        function _emscripten_resize_heap(requestedSize) { var oldSize = _emscripten_get_heap_size(); var PAGE_MULTIPLE = 65536; var LIMIT = 2147483648 - PAGE_MULTIPLE; if (requestedSize > LIMIT) {
            return false;
        ...
        var ENV = {};
        function _emscripten_get_environ() { if (!_emscripten_get_environ.strings) {
        ...
        function _environ_get(__environ, environ_buf) { var strings = _emscripten_get_environ(); var bufSize = 0; strings.forEach(function (string, i) { var ptr = environ_buf + bufSize; HEAP32[__environ + i * 4 >> 2] = ptr; writeAsciiToMemory(string, ptr); bufSize += string.length + 1; }); return 0; }
        ...
        var ___tm_current = 357776;
        var ___tm_timezone = (stringToUTF8("GMT", 357824, 4), 357824);
        ...
        function _nanosleep(rqtp, rmtp) { if (rqtp === 0) {
            ___setErrNo(28);
            return -1;
        } var seconds = HEAP32[rqtp >> 2]; var nanoseconds = HEAP32[rqtp + 4 >> 2]; if (nanoseconds < 0 || nanoseconds > 999999999 || seconds < 0) {
            ___setErrNo(28);
            return -1;
        } if (rmtp !== 0) {
            HEAP32[rmtp >> 2] = 0;
        ...
        function _uuid_generate(out) { var uuid = null; if (ENVIRONMENT_IS_NODE) {
            try {
        ...
        var __ZSt18uncaught_exceptionv = Module["__ZSt18uncaught_exceptionv"] = function () { return Module["asm"]["_ZSt18uncaught_exceptionv"].apply(null, arguments); };
        var ___getTypeName = Module["___getTypeName"] = function () { return Module["asm"]["__getTypeName"].apply(null, arguments); };
        ...
        var __growWasmMemory = Module["__growWasmMemory"] = function () { return Module["asm"]["__growWasmMemory"].apply(null, arguments); };
        ...
        var dynCall_viiddd = Module["dynCall_viiddd"] = function () { return Module["asm"]["dynCall_viiddd"].apply(null, arguments); };
        var dynCall_iiiddd = Module["dynCall_iiiddd"] = function () { return Module["asm"]["dynCall_iiiddd"].apply(null, arguments); };
        ...
        var dynCall_iiiidd = Module["dynCall_iiiidd"] = function () { return Module["asm"]["dynCall_iiiidd"].apply(null, arguments); };
        var dynCall_viidd = Module["dynCall_viidd"] = function () { return Module["asm"]["dynCall_viidd"].apply(null, arguments); };
        var dynCall_iidid = Module["dynCall_iidid"] = function () { return Module["asm"]["dynCall_iidid"].apply(null, arguments); };
        var dynCall_iiidid = Module["dynCall_iiidid"] = function () { return Module["asm"]["dynCall_iiidid"].apply(null, arguments); };
        var dynCall_iiidddddd = Module["dynCall_iiidddddd"] = function () { return Module["asm"]["dynCall_iiidddddd"].apply(null, arguments); };
        ...
        var dynCall_iiiddiiidd = Module["dynCall_iiiddiiidd"] = function () { return Module["asm"]["dynCall_iiiddiiidd"].apply(null, arguments); };
        var dynCall_iiididdddd = Module["dynCall_iiididdddd"] = function () { return Module["asm"]["dynCall_iiididdddd"].apply(null, arguments); };
        ...
        var dynCall_iiiiddiiidd = Module["dynCall_iiiiddiiidd"] = function () { return Module["asm"]["dynCall_iiiiddiiidd"].apply(null, arguments); };
        var dynCall_iiiddiddddd = Module["dynCall_iiiddiddddd"] = function () { return Module["asm"]["dynCall_iiiddiddddd"].apply(null, arguments); };
        var dynCall_iiddiiiii = Module["dynCall_iiddiiiii"] = function () { return Module["asm"]["dynCall_iiddiiiii"].apply(null, arguments); };
        var dynCall_iiddii = Module["dynCall_iiddii"] = function () { return Module["asm"]["dynCall_iiddii"].apply(null, arguments); };
        var dynCall_iiiidiii = Module["dynCall_iiiidiii"] = function () { return Module["asm"]["dynCall_iiiidiii"].apply(null, arguments); };
        var dynCall_iididii = Module["dynCall_iididii"] = function () { return Module["asm"]["dynCall_iididii"].apply(null, arguments); };
        ...
        function run(args) { args = args || arguments_; if (runDependencies > 0) {
            ...
            return; calledRun = true; if (ABORT)
...
//# sourceMappingURL=rhino3dm.js.map