define(["require", "exports", "../../../../build/three.module.js", "../core/TempNode.js", "../core/ConstNode.js", "../inputs/FloatNode.js", "../core/FunctionNode.js", "../core/ExpressionNode.js"], function (require, exports, three_module_js_1, TempNode_js_1, ConstNode_js_1, FloatNode_js_1, FunctionNode_js_1, ExpressionNode_js_1) {
    "use strict";
    ...
    ColorSpaceNode.Nodes = (function () {
        ...
        var LinearToLinear = new FunctionNode_js_1.FunctionNode([
            "vec4 LinearToLinear( in vec4 value ) {",
            "	return value;",
            "}"
        ].join("\n"));
        var GammaToLinear = new FunctionNode_js_1.FunctionNode([
            "vec4 GammaToLinear( in vec4 value, in float gammaFactor ) {",
            "	return vec4( pow( value.xyz, vec3( gammaFactor ) ), value.w );",
            "}"
        ].join("\n"));
        var LinearToGamma = new FunctionNode_js_1.FunctionNode([
            "vec4 LinearToGamma( in vec4 value, in float gammaFactor ) {",
            "	return vec4( pow( value.xyz, vec3( 1.0 / gammaFactor ) ), value.w );",
            "}"
        ].join("\n"));
        var sRGBToLinear = new FunctionNode_js_1.FunctionNode([
            "vec4 sRGBToLinear( in vec4 value ) {",
            "	return vec4( mix( pow( value.rgb * 0.9478672986 + vec3( 0.0521327014 ), vec3( 2.4 ) ), value.rgb * 0.0773993808, vec3( lessThanEqual( value.rgb, vec3( 0.04045 ) ) ) ), value.w );",
            "}"
        ].join("\n"));
        var LinearTosRGB = new FunctionNode_js_1.FunctionNode([
            "vec4 LinearTosRGB( in vec4 value ) {",
            "	return vec4( mix( pow( value.rgb, vec3( 0.41666 ) ) * 1.055 - vec3( 0.055 ), value.rgb * 12.92, vec3( lessThanEqual( value.rgb, vec3( 0.0031308 ) ) ) ), value.w );",
            "}"
        ].join("\n"));
        var RGBEToLinear = new FunctionNode_js_1.FunctionNode([
            "vec4 RGBEToLinear( in vec4 value ) {",
            "	return vec4( value.rgb * exp2( value.a * 255.0 - 128.0 ), 1.0 );",
            "}"
        ].join("\n"));
        var LinearToRGBE = new FunctionNode_js_1.FunctionNode([
        ...
        var RGBMToLinear = new FunctionNode_js_1.FunctionNode([
            "vec3 RGBMToLinear( in vec4 value, in float maxRange ) {",
            "	return vec4( value.xyz * value.w * maxRange, 1.0 );",
            "}"
        ].join("\n"));
        var LinearToRGBM = new FunctionNode_js_1.FunctionNode([
        ...
        // reference: http://iwasbeingirony.blogspot.ca/2010/06/difference-between-rgbm-and-rgbd.html
        var RGBDToLinear = new FunctionNode_js_1.FunctionNode([
            "vec3 RGBDToLinear( in vec4 value, in float maxRange ) {",
            "	return vec4( value.rgb * ( ( maxRange / 255.0 ) / value.a ), 1.0 );",
            "}"
        ].join("\n"));
        var LinearToRGBD = new FunctionNode_js_1.FunctionNode([
        ...
        var cLogLuvM = new ConstNode_js_1.ConstNode("const mat3 cLogLuvM = mat3( 0.2209, 0.3390, 0.4184, 0.1138, 0.6780, 0.7319, 0.0102, 0.1130, 0.2969 );");
        var LinearToLogLuv = new FunctionNode_js_1.FunctionNode([
        ...
        var cLogLuvInverseM = new ConstNode_js_1.ConstNode("const mat3 cLogLuvInverseM = mat3( 6.0014, -2.7008, -1.7996, -1.3320, 3.1029, -5.7721, 0.3008, -1.0882, 5.6268 );");
        var LogLuvToLinear = new FunctionNode_js_1.FunctionNode([
    ...
    ColorSpaceNode.getEncodingComponents = function (encoding) {
        switch (encoding) {
            case three_module_js_1.LinearEncoding:
                return ['Linear'];
            case three_module_js_1.sRGBEncoding:
                return ['sRGB'];
            case three_module_js_1.RGBEEncoding:
                return ['RGBE'];
            case three_module_js_1.RGBM7Encoding:
                return ['RGBM', new FloatNode_js_1.FloatNode(7.0).setReadonly(true)];
            case three_module_js_1.RGBM16Encoding:
                return ['RGBM', new FloatNode_js_1.FloatNode(16.0).setReadonly(true)];
            case three_module_js_1.RGBDEncoding:
                return ['RGBD', new FloatNode_js_1.FloatNode(256.0).setReadonly(true)];
            case three_module_js_1.GammaEncoding:
                return ['Gamma', new ExpressionNode_js_1.ExpressionNode('float( GAMMA_FACTOR )', 'f')];
        }
    };
    ColorSpaceNode.prototype = Object.create(TempNode_js_1.TempNode.prototype);
