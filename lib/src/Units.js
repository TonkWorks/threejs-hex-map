define(["require", "exports", "three", "./CSS3DRenderer", "./Nations", "./util"], function (require, exports, three_1, CSS3DRenderer_1, Nations_1, util_1) {
    ...
    exports.CreateResourceMapModel = CreateResourceMapModel;
    ...
    function createTileOverlayModel(color = "white", opacity = 0.2) {
    ...
    function CreateResourceModel(resource) {
        ...
        texture.magFilter = three_1.NearestFilter;
        const iconModel = new three_1.Mesh(new three_1.BoxBufferGeometry(.4, .4, .001), new three_1.MeshBasicMaterial({
            // color: player.color,
            map: texture,
            transparent: true,
            side: three_1.FrontSide,
        ...
        if (resource.map) {
            ...
            mapTexture.magFilter = three_1.NearestFilter;
            mapModel = new three_1.Mesh(new three_1.PlaneBufferGeometry(1, 1), new three_1.MeshBasicMaterial({
                map: mapTexture,
                transparent: true,
                side: three_1.FrontSide,
                alphaTest: .5,
        ...
        return {
            ...
            model: iconModel,
            mapModel: mapModel,
            improvement_needed: resource.improvement_needed,
        };
    }
    function CreateResourceMapModel(resource) {
        const textureLoader = new three_1.TextureLoader();
        // map
        if (!resource.map) {
            return null;
        ...
        const mapTexture = textureLoader.load(`${resource.map}`);
        mapTexture.magFilter = three_1.NearestFilter;
        const mapModel = new three_1.Mesh(new three_1.PlaneBufferGeometry(2, 2), new three_1.MeshBasicMaterial({
            // color: player.color,
            map: mapTexture,
            transparent: true,
            side: three_1.FrontSide,
