...
define(["require", "exports", "three", "./MapMesh", "./interfaces", "./util", "./Grid", "./DefaultTileSelector", "./DefaultTileHoverSelector", "./Units", "./DefaultMapViewController", "./coords", "./ChunkedLazyMapMesh", "./ParticleSystemEffects", "./ParticleSystem", "./Units", "./GameState", "./CSS3DRenderer", "./AI", "./toastify", "./Nations", "./Research", "./Selector", "./third/postprocessing.esm", "./third/stats"], function (require, exports, three_1, MapMesh_1, interfaces_1, util_1, Grid_1, DefaultTileSelector_1, DefaultTileHoverSelector_1, Units_1, DefaultMapViewController_1, coords_1, ChunkedLazyMapMesh_1, ParticleSystemEffects_1, ParticleSystem_1, Units_2, GameState_1, CSS3DRenderer_1, AI_1, toastify_1, Nations_1, Research_1, Selector_1, postprocessing_esm_1, Stats) {
    ...
    class MapView {
        constructor(canvasElementQuery = "canvas") {
            ...
            this._minimap_aspect_height = 300;
            this.composer = null;
            this.stats = null;
            ...
            this.animate = (timestamp) => {
                ...
                this._minimap_camera.position.z = this._tileGrid.width * 2.45;
                this._onAnimate(dtS);
        ...
        hoverTile(tile, x, y) {
            ...
            if (enemyTargeted && this.selectedTile.unit && this.selectedTile.unit.owner === this.gameState.currentPlayer) {
                ...
                if (distance <= this.selectedTile.unit.attack_range) {
                    const arrow = new DefaultTileHoverSelector_1.AttackArrow(coords_1.qrToWorld(this.selectedTile.q, this.selectedTile.r), 3.5, worldPos);
                ...
                const path = this.calculatePath(this.selectedTile, tile, this.selectedTile.unit.movement);
                this.showPath(path);
            }
        ...
        selectTile(tile) {
            ...
            if (this.placingCity) {
                ...
                let defaultName = Units_1.getNextCityName(player);
                this.getTextInput(`<h3>City Name</h3>`, defaultName, (name) => {
                    let player = this.getPlayer(this.gameState.currentPlayer);
                    let city = Units_2.CreateCity(player, name);
        ...
        getNextMovementTile(unit, current, target) {
            ...
            for (const neighbor of neighbors) {
                ...
                if (unit.land == false && !interfaces_1.isWater(tt.height)) {
                    continue;
                }
                if (unit.water == false && interfaces_1.isWater(tt.height)) {
                    continue;
                }
        ...
        showPath(path) {
            ...
            this._pathIndicators.clear();
            this._pathIndicators.children.forEach(child => this._pathIndicators.remove(child));
            // Add path indicators for each tile in the path
            path.forEach(tile => {
                const worldPos = coords_1.qrToWorld(tile.q, tile.r);
                const geometry = new three_1.RingBufferGeometry(0.01, 1, 6, 0.2);
                geometry.rotateZ(Math.PI / 2);
                const material = DefaultTileHoverSelector_1.hoverSelectorMaterial.clone();
                material.color.set(0x00ff00); // Green color for path
                material.opacity = 1;
                const selector = new three_1.Mesh(geometry, material);
                selector.scale.set(0.9, 0.9, 1);
                selector.position.set(worldPos.x, worldPos.y, 0.02);
                this._pathIndicators.add(selector);
            });
            // Add step count on the last tile
            if (path.length > 0) {
                const lastTile = path[path.length - 1];
                const worldPos = coords_1.qrToWorld(lastTile.q, lastTile.r);
                ...
                context.fillText(path.length.toString(), canvas.width / 2, canvas.height / 2);
                // Create sprite with text texture
                const texture = new three_1.CanvasTexture(canvas);
                const spriteMaterial = new three_1.SpriteMaterial({ map: texture });
                const sprite = new three_1.Sprite(spriteMaterial);
                sprite.position.set(worldPos.x, worldPos.y, 0.03); // Higher Z to be above path rings
