    ...
    class MapView {
        ...
        updateAllVisilibility() {
            ...
            for (const tile of revealed_natural_wonders) {
                ...
                this.menuQueue.push(() => {
                    sound = (0, util_1.asset)("sounds/units/barb-hit.mp3");
                    let n = ImprovementsNaturalWonders_1.NaturalWonderMap[tile.naturalWonder.type];
        ...
        addNaturalWonderToMap(naturalWonder, tile) {
            const worldPos = (0, coords_1.qrToWorld)(tile.q, tile.r);
        ...
        initGameSetup() {
            if (this._savedGame) {
                ...
                for (const tile of this._tileGrid.toArray()) {
                    ...
                    this.addNaturalWonderToMap((0, ImprovementsNaturalWonders_1.CreateNaturalWonder)(tile.naturalWonder.type), tile);
                }
            ...
            for (let tile of this._tileGrid.toArray()) {
                tile.yields = { "gold": 0 };
                switch ((0, interfaces_1.getTerrain)(tile)) {
                    case 'grass':
            ...
            for (const [key, NaturalWonder] of Object.entries(ImprovementsNaturalWonders_1.NaturalWonderMap)) {
                let tile = this.getRandomTile(true);
                this.addNaturalWonderToMap((0, ImprovementsNaturalWonders_1.CreateNaturalWonder)(key), tile);
            }
            this.updateGlobalFog();
            this.addUnitSelectors();
            this.showTurnBanner();
        ...
        updateToolTip(tile, x, y) {
            ...
            tooltip.innerHTML = "";
            let data = [];
            data.push(`<div class="panel-title">${(0, util_1.capitalize)((0, interfaces_1.getTerrain)(tile))}</div>`);
        ...
        generateResourceInfo(tile) {
            ...
            <div>
                ...
                <div style="display: flex; align-items: left;">
                    <table style="margin-right: 10px; text-align: left;">
                        <tr>
                            <td>${(0, util_1.capitalize)(tile.resource.name)}</br>${resourceIcon(tile.resource.name)}</br>${info}</td>
                        </tr>
                    </table>
                </div>
            </div>`;
        ...
        eligibleImprovements(tile) {
            ...
            else if ((0, interfaces_1.isWater)(tile.height)) {
            ...
            if (tile.unit && shouldShowUnit && tile.unit === this._selectedUnit) {
                ...
                if (tile.unit.type === "worker") {
                    ...
                    for (const impType of eligibleImprovements) {
                        ...
                        let disabled = ``;
                        let improvementType = ImprovementsWorker_1.WorkerImprovementMap[impType];
        ...
        `;
            if (completed) {
                let tech = Research_1.Technologies.get(completed);
                research_info += `    
            ...
            let options = ``;
            Research_1.Technologies.forEach(tech => {
                if ((0, Research_1.canUnlock)(tech) === false) {
                    return;
                }
        ...
        cityMenuAction(name, target, target_em) {
            ...
            if (name === "pick_research") {
                let tech = Research_1.Technologies.get(target);
                if (tech === undefined) {
            ...
            if (name === "worker_improvement") {
                ...
                if (tile.resource) {
                    const mm = this;
                    this.playSound((0, util_1.asset)("sounds/ui/drop.mp3"));
                    let city_name = player.improvements[tile.city].name;
                    this.toast({
                        icon: `../../assets/map/resources/map/${tile.resource.name}.png`,
                        text: `${(0, util_1.capitalize)(tile.resource.name)} has been connected to ${city_name}!`,
                        onClick: function () { }
                    });
                }
                this.focusOnNextAction();
