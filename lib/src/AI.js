...
define(["require", "exports", "./CityImprovements", "./ImprovementsWorker", "./interfaces", "./Units"], function (require, exports, CityImprovements_1, ImprovementsWorker_1, interfaces_1, Units_1) {
    "use strict";
    ...
    exports.findTargetForWorker = findTargetForWorker;
    function sleep(ms) {
    ...
    function buildEconomy(mapView, player) {
        ...
        for (const [key, improvement] of Object.entries(player.improvements)) {
            if (improvement.production_queue.length === 0) {
                ...
                if (rec !== "") {
                    ...
                    if (Object.keys(CityImprovements_1.BuildingMap).includes(rec)) {
    ...
    // Barbarian AI
    function BarbarianUnitAI(mapView, player) {
        for (const unit of Object.values(player.units)) {
            ...
            if (t.worker_improvement && t.worker_improvement.type === "encampent") {
                ...
                if (randomNumber === 0) {
                    // spawn a new unit
                    const newUnit = (0, Units_1.createUnit)("rifleman", player);
    ...
    function placeBarbarianEncampents(mapView) {
        ...
        for (const t of tiles) {
            ...
            let terrain = (0, interfaces_1.getTerrain)(t);
            ...
            if (mapView.getTileGrid().neighbors(t.q, t.r, 6).filter(x => { var _a; return ((_a = x.worker_improvement) === null || _a === void 0 ? void 0 : _a.type) === "encampent"; }).length > 0) {
                continue;
            }
        ...
        randomTile.worker_improvement = (0, ImprovementsWorker_1.CreateWorkerImprovement)("encampent");
        ...
        mapView.addUnitToMap((0, Units_1.createUnit)("rifleman", barbPlayer), t);
    }
    ;
    // General AI
    function getRecommendedEconomic(improvement, player) {
    ...
    function findTargetForWorker(mapView, tile, tilesBeingWorked) {
        ...
        for (const t of tt) {
            ...
            if (tile !== target) {
                ...
                mapView.addWorkerImprovementToMap((0, ImprovementsWorker_1.CreateWorkerImprovement)(type), t);
            }
            return t;
        }
    }
    function hasEnoughWorkers(player) {
        let count = 0;
        for (const [key, unit] of Object.entries(player.units)) {
            if (unit.type === "worker") {
                count += 1;
